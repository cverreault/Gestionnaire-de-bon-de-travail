...
- Notes terrain + pièces jointes (relations dédiées) — images, documents et **vidéos** (B56 : mp4/mov/webm/3gp, 100 Mo max, magic bytes vérifiés, `client_max_body_size 120M` côté nginx ; l'app filme jusqu'à 3 min, **compresse sur l'appareil** (B69, `react-native-compressor` : H.264 ≤ 1920 px, 5 Mbit/s, encodeurs matériels, hors ligne) puis met la vidéo en file ; la limite de 100 Mo porte sur le fichier compressé ; **B64** : après l'upload, `VideoTranscodeService` ré-encode en arrière-plan avec ffmpeg — H.264 ≤ 1920 px, CRF 23, AAC 96 kb/s, faststart — et remplace l'objet et la ligne seulement si le fichier rétrécit d'au moins 15 % (un clip 1080p de 6 s passe de 6,5 Mo à 0,66 Mo) ; `ATTACHMENTS_VIDEO_TRANSCODE=0` désactive, `ATTACHMENTS_VIDEO_MAX_WIDTH` / `ATTACHMENTS_VIDEO_CRF` ajustent)
