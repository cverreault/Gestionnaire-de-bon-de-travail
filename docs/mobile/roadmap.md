...
| B38.6 ✅ | `feat(mobile)` | Caméra / galerie, redimension 1600 px JPEG 0.8, copie dans le sandbox et upload par la file hors ligne, vignettes via le proxy ; signature client et technicien (canevas WebView → PNG data-URL) en file, `expectedUpdatedAt` sur `POST /:id/signatures` (409 → pull et rejeu) | B37.9 |
| B38.7 ✅ | `feat(mobile)` | Catalogue et stock locaux (pull), carte Pièces sur le détail (recherche locale SKU / nom, scan de code-barres expo-camera, quantité, camion / entrepôt), opérations `part_add` (additive) et `part_remove` (bloquante) dans la file, onglet Mon stock | B37.6 |
| B38.8 ✅ | `feat(mobile)` | Consentement sur le profil (préférence serveur + permissions OS « pendant l'utilisation » / « toujours »), règle pure `decideTracking` (consentement × permission × BT en route ou en cours), tâche expo-location d'arrière-plan qui n'écrit que dans `location_fixes`, envoi par lots de 100 au premier plan et après chaque sync (`Idempotency-Key` par lot), 403 → arrêt et avis « désactivé depuis le web » | B37.7 |
| B38.9 ✅ | `feat(mobile)` | expo-notifications (permission, canal Android), token Expo envoyé à l'enregistrement et au heartbeat (ré-enregistrement quand il change), pull à la réception, navigation vers le BT au toucher (`data.workOrderId` ou `data.url`), état sur le profil ; le token exige un projet EAS initialisé (`EAS_PROJECT_ID`) | B37.4 |
| B38.10 ✅ | `chore(mobile)` | `eas.json` (development / preview / production, `appVersionSource: local`, `autoIncrement`), flows Maestro (connexion, note hors ligne puis drain), [`docs/mobile/release.md`](release.md) (init EAS, credentials, déclarations de store, porte de version, checklist) ; EAS Update non activé en v1 | B38.3 à B38.9 |
| B38.11 ✅ | `feat(frontend)` | QR code de l'espace sur `/profil` (origine courante encodée côté client, aucun endpoint) et bouton « Scanner » sur l'écran workspace de l'app | B38.3 |
