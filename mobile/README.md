...
Chaque geste du technicien (transition, note, photo) est écrit dans `sync_queue` puis envoyé par le drain séquentiel (`src/sync/drain.ts`) avec `Idempotency-Key = id de l'opération` et `expectedUpdatedAt` propagé. Sur 409 de verrou optimiste, l'app tire d'abord, rejoue les opérations additives (3 essais) et met les transitions en conflit ; l'onglet Sync permet d'appliquer quand même ou d'abandonner (ADR-016 §4). Les photos sont copiées dans `documentDirectory/queue/` en attendant l'envoi. Les signatures (client, technicien) sont capturées par `SignaturePad` (react-native-signature-canvas sur WebView) en PNG data-URL et passent par la même file.
